...
export async function GET(_r: NextRequest, { params }: { params: { id: string } }) {
  const { ok, status, data } = await apiServerFetch(`/activites/${params.id}/pieces-jointes`);
  return NextResponse.json(data ?? { message: 'Erreur' }, { status: ok ? 200 : status });
}
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  const formData = await request.formData();
  const res = await apiServerRaw(`/activites/${params.id}/pieces-jointes`, { method: 'POST', body: formData });
